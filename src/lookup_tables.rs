...
#[repr(C, align(32))]
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct SimdTableI64<const U: usize>([i32; U]);

impl<const U: usize> SimdTableI64<U>
where
    [i32; U]: Sized,
{
    pub fn new(data: [i32; U]) -> Self {
            ...
            if has_avx2() {
                while processed + 8 <= Y {
                    let chunk_offsets =
                        _mm256_loadu_si256(offsets.as_ptr().add(processed) as *const __m256i);
                    let chunk_result =
                        _mm256_i64gather_epi64::<4>(self.0.as_ptr() as *const i64, chunk_offsets);
                    _mm256_storeu_si256(
                        result.as_mut_ptr().add(processed) as *mut __m256i,
                        chunk_result,
                    );
            ...
            if has_avx() {
                while processed + 4 <= Y {
                    let chunk_offsets =
                        _mm_loadu_si128(offsets.as_ptr().add(processed) as *const __m128i);
                    let chunk_result =
                        _mm_i64gather_epi64::<4>(self.0.as_ptr() as *const i64, chunk_offsets);
                    _mm_storeu_si128(
                        result.as_mut_ptr().add(processed) as *mut __m128i,
                        chunk_result,
                    );
